...
from utils.ablation import Ablation
from utils.sql import SQL
from utils.faiss import Faiss


class AblationProcess(Ablation):
    def __init__(self, lang):
        super().__init__()
        self.conv_log_path_1 = os.path.abspath(os.path.join(os.path.dirname(__file__), '../datasets/SMRCslog/conv_log_1.txt'))
        ...
        if lang == 'en':
            ...
            self.ablation_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../ablation/SMRCs (EN)/'))
        elif lang == 'ja':
            ...
            self.ablation_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../ablation/SMRCs (JA)/'))
        else:
            raise ValueError(f"Unsupported language: {lang}")

    def ablation(self, sr_params, ab_stim, ab_lif, ab_prop):
        if ab_stim:
            param = 'stim'
        elif ab_lif:
            param = 'lif'
        ...
        def process_section(section):
            ...
            for dialogue in dialogues:
                ...
                sr_results = self.propagate_stimuli_ab(faiss, sr_sql, data, cos_th, v_th, stimulus_th, tau_init, tau_scale, time_scale, bond_scale, v_rest, i_rest, ab_stim, ab_lif, ab_prop)
        ...
        with open(self.ablation_dir + f'/{param}.json', 'w') as f:
    ...
    def sr_opt_run(self, trial, count_th, sr_params, ab_stim, ab_lif, ab_prop):
        cos_th = sr_params['cos_th']
        v_th = sr_params['v_th']
        stimulus_th = sr_params['stimulus_th']
        tau_init = sr_params['tau_init']
        ...
        for section in range(section_count):
            ...
            for dialogue in dialogues:
                recall_indices = dialogue['recall']
                data = self.prepare_data(faiss, sr_sql, dialogue['index'], dialogue['vector'])
                sr_results = self.propagate_stimuli_ab(faiss, sr_sql, data, cos_th, v_th, stimulus_th, tau_init, tau_scale, time_scale, bond_scale, v_rest, i_rest, ab_stim=ab_stim, ab_lif=ab_lif, ab_prop=ab_prop)
                if len(recall_indices) > 0:
