...
def sr_objective(trial, eval_process):
    ...
    v_th = trial.suggest_float('v_th', 0.0, 0.1)
    stimulus_th = trial.suggest_float('stimulus_th', 0.0, 0.1)
...
def run(model):
    ...
    params_dict = {
        'sr': parameters.sr_params,
        'ma': parameters.ma_params,
        'ma_ex': parameters.ma_ex_params,
        'mb_ex': parameters.mb_ex_params
    }

    for trial in best_trials:
        eval_params = [params_dict[key] if key != model else trial.params for key in ['sr', 'ma', 'ma_ex', 'mb_ex']]
        print(trial.params)
        eval_score = eval_process.eval_run(*eval_params)[model]
        if eval_score > best_eval_score:
            best_eval_score = eval_score
            best_eval_params = trial.params
